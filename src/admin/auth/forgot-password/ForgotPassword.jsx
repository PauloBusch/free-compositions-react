...
class ForgotPassword extends Component {
  ...
  render() {
    const { handleSubmit, forgotPassword, sendedForgotPasswordEmail, sendedForgotPasswordEmailLoading } = this.props;

    return (
      <div className="background-forgot-password">
        <If test={ !sendedForgotPasswordEmail }>
          <Form id="form-forgot-password" onSubmit={ handleSubmit(forgotPassword) }>
            <h2>Redefinir Senha</h2>
            <Field component={ Input } type="email" name="email"
              placeholder="E-mail" icon="user"/>
            <SubmitButton loading={ sendedForgotPasswordEmailLoading } disabled={ !this.isValid() } fill padding="10px" text="Enviar Email"/>
          </Form>
        </If>
        <If test={ sendedForgotPasswordEmail }>
...
const forgotPasswordForm = reduxForm({ form: 'forgot-password-form' })(withRouter(ForgotPassword));
